...
KOMI = 7.5                         # 贴目（白方补偿）
MAX_MOVES = 380                    # 单局最大手数（防死循环）
PASS_MOVE = (-1, -1)               # 停一手标记
MIN_MOVES_BEFORE_PASS = 280        # 前 N 手不允许 Pass
SCALE = 10                         # 贴目/目差归一化缩放
PASS_LIMIT = 5                     # 连续 Pass 上限（终局判定）
...
TEMPERATURE_DECAY = 0.99           # 温度指数衰减系数
TEMPERATURE_ZERO_AFTER = 60        # 训练温度调度
...
ENTROPY_WEIGHT = 0.15              # 策略熵正则权重
OWN_WEIGHT = 2.5                   # 领地头损失权重
...
TRAINING_DATA_MAX = 120000          # 训练数据缓冲上限（deque maxlen）
...
TRT_OPT_LEVEL = 3                  # TRT builder 优化级别（3 级：构建快、内存小）
